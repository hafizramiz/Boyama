...
class QueueLinearFloodFiller {
  ...
  void setTargetColor(img.Pixel targetColor) {
    _startColor[0] = targetColor.r.toInt();
    _startColor[1] = targetColor.g.toInt();
    _startColor[2] = targetColor.b.toInt();
    _startColor[3] = targetColor.a.toInt();
  }
  ...
  Future<void> floodFill(int x, int y) async {
    ...
    if (_startColor[0] == 0) {
      // ***Get starting color.
      img.Pixel startPixel = image!.getPixel(x, y);

      _startColor[0] = startPixel.r.toInt();
      _startColor[1] = startPixel.g.toInt();
      _startColor[2] = startPixel.b.toInt();
    }
  ...
  void _linearFill(int x, int y) {
    ...
    while (true) {
      ...
      img.Color pixelColor = img.ColorRgba8(
        (_fillColor >> 0) & 0xFF,   // red
        (_fillColor >> 8) & 0xFF,   // green  
        (_fillColor >> 16) & 0xFF,  // blue
        (_fillColor >> 24) & 0xFF   // alpha
      );
      image?.setPixel(lFillLoc, y, pixelColor);
    ...
    while (true) {
      ...
      image?.setPixel(rFillLoc, y, pixelColor);
