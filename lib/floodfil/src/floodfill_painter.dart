...
class FloodFillPainter extends CustomPainter {
  ...
  void _initFloodFiller() async {
    ...
    // Convert Color to RGBA int representation
    int colorValue = (fillColor.alpha << 24) | (fillColor.blue << 16) | (fillColor.green << 8) | fillColor.red;
    _filler = QueueLinearFloodFiller(decoded, colorValue);
    onInitialize!();
  }
  ...
  void setFillColor(Color color) {
    // Convert Color to RGBA int representation
    int colorValue = (color.alpha << 24) | (color.blue << 16) | (color.green << 8) | color.red;
    _filler?.setFillColor(colorValue);
  }
  ...
  bool _checkAvoidColor(img.Pixel touchColor) {
    if (_avoidColor == null) return false;

    return _avoidColor!.any((element) => _isAvoidColor(element, touchColor));
  }

  bool _isAvoidColor(Color avoidColor, img.Pixel touchColor) {
    int touchR = touchColor.r.toInt();
    int touchG = touchColor.g.toInt();
    int touchB = touchColor.b.toInt();
    int touchA = touchColor.a.toInt();
  ...
  void fill(Offset position) async {
    ...
    img.Pixel touchColor = _filler!.image!.getPixel(pX, pY);
